...
We’ll use: tidyverse and scales to create plots in this exercise.
...
Now that we have identified how this graph is misleading, let's create our own graph that fixes the issue!
The first thing we need to do is think about the structure of our data.
As seen above, our three column names are `date`, `mask mandate`, and `no-mask mandate`.
...
The values of mask mandate and no-mask mandate need to be levels of a categorical variable that we can map to the visual element (lines) on our plot.
In the following code below, we name this variable `County`.
...
Now that our data is in the correct format (note that we also changed our `date` variable to be in the format of month, day, year), we are going to use `ggplot()`, and the appropriate geom function to make our new graph.
Please see the [Function Reference](https://ggplot2.tidyverse.org/reference/) for the different types of `geoms` we can use.
Add the appropriate geom to the plot below to make a line graph.
...
Based on our new graph, we can see the Kansas Covid-19 7 day rolling averages per/100k between the dates of 7/12/2020 through 8/03/2020 decreased for the mask mandated counties, and stayed relatively consistent for the non-masked mandated counties.
...
We can add the following lines of code to make the graph even more effective.
We are using `scale_color_manual()` to change the colors of the line associated with the County variable; `scale_x_date()`to add date labels to the x-axis; `theme()`to change the angle of the x-axis and the position of the legend; and `labs()` to add a title!
We also have given a subset of dates, and removed having an angled x-axis to improve readability for the user.
...
kansas_covid |>
  ...
  geom_line() +
  scale_color_manual(
    values = c("mask mandate" = "#E69F00", "no-mask mandate" = "#0072B2")) +
...
Georgia DPH temporarily published the following graph on the COVID-19 Daily Status Report.
The data and associated plot can be found below.
These data are pulled from the NY Times.
...
One strategy we can use in R is to manually set the dates in chronological order using the `levels` argument in the `factor()` function when manipulating the `date` variable in our data set using `mutate()`.
...
We can use the following code to help make their plot less misleading.
We will not sort the levels of county within each date to be in descending order, to make comparisons easier.
The function `geom_col()` is used to re-create the barplot.
The functions `theme()`, `scale_x_discrete()`, and `scale_fill_manual()` are used to help recreate some of the visual features of the plot above.
